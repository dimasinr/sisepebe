...
class Command(BaseCommand):
    help = "Import data siswa dan penilaian kelas XII dari file Excel (f_leger_XII A.xlsx / f_leger_XII B.xlsx)"

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='f_leger_XII B.xlsx',
            help='Path ke file Excel leger nilai kelas XII'
        )
        parser.add_argument(
            '--run-seleksi',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        # 1. Tentukan Sheet Nilai (Worksheet) dan Sheet Sosial (Sheet2/Sheet1)
        ...
        for r in range(8, ws.max_row + 1):
            ...
            # Tentukan jenis kelamin (digit ke-5 NIS: 1=L, 2=P)
            jk = 'L' if len(nis) >= 5 and nis[4] == '1' else 'P'
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\nSelesai! Berhasil mengimpor {created_count} siswa baru dan memperbarui {updated_count} siswa kelas XII."
        ))
        ...
        if options['run_seleksi']:
            ...
            self.stdout.write(self.style.NOTICE("\nMenjalankan perhitungan C4.5 untuk siswa Kelas XII..."))
